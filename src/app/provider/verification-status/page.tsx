...
const pendingActions: PendingAction[] = [
  {
    id: "profile-photo",
    title: "Profile Photo",
    fix: "Image is too blurry. Upload a clear, well-lit photo of your face.",
    cta: "Retake Photo",
    icon: Camera,
  },
  {
    id: "aadhaar-back",
    title: "Aadhaar Card",
    fix: "The back side of your Aadhaar card is missing. Upload it to continue.",
    cta: "Upload Back",
    icon: Upload,
  },
];
...
const timeline: TimelineStep[] = [
  {
    ...
    body: "We have received your initial application.",
    state: "done",
  },
  {
    title: "Verification Review",
    body: "Our team is reviewing your documents.",
    state: "current",
  },
  {
    title: "Start Earning",
...
export default function ProviderVerificationStatusPage() {
  ...
  const [resolved, setResolved] = useState<Record<ActionId, boolean>>({
    "profile-photo": false,
    "aadhaar-back": false,
  });

  const remaining = useMemo(
    () => pendingActions.filter((action) => !resolved[action.id]).length,
    [resolved],
  );
      ...
      <div className="mx-auto flex min-h-dvh w-full min-w-0 max-w-[480px] flex-col overflow-x-hidden bg-[var(--surface)] pb-[calc(156px+env(safe-area-inset-bottom))]">
        ...
        <section className="flex flex-col gap-6 px-4 py-5 min-[390px]:px-5 min-[390px]:py-6">
          <div className="flex flex-col items-center rounded-xl border border-[var(--outline-variant)] bg-[var(--surface-container-low)] p-6 text-center">
            ...
            <h2 className="mt-3 text-headline-md text-[var(--on-surface)]">
              Pending Review
            </h2>
            <p className="mt-1.5 max-w-[20rem] text-body-sm text-[var(--on-surface-variant)]">
              We are verifying your details. Reviews usually take 24-48 hours after resubmission.
              Please complete the exact fixes below to avoid delays.
            </p>
          ...
          <div className="flex flex-col gap-3">
            ...
            {pendingActions.map((action) => {
              const isResolved = resolved[action.id];
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <h4 className="text-label-lg text-[var(--on-surface)]">
                      {action.title}
                    </h4>
